...
func conexionDB_Despacho() (conexion *sql.DB) {
	Driver := "mysql"
	Usuario := "admin"
	Contrasenia := "12345678"
	DB := "db_despachos"
...
var contextDespacho = conexionDB_Despacho()
...
func stateDespacho(c *gin.Context) {
	id := c.Param("id")
	var newDespacho despacho
	data, _ := contextDespacho.Query("SELECT * FROM despacho WHERE id_despacho=?", id)
	data.Next()
	data.Scan(&newDespacho.Id_despacho, &newDespacho.Estado, &newDespacho.Id_compra)
	c.IndentedJSON(http.StatusOK, newDespacho)
